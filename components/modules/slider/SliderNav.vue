...
<template>
  ...
  <div
    class="slider-nav bottom-0 p-6 absolute justify-center w-full z-50 flex items-center"
    dir="ltr"
  >
    <!-- Previous button -->
    <div
      class="swiper-circle cursor-pointer swiper-prev group"
    ...
    >
      ...
      >
        ...
        <svg
          class="circle h-[45px] w-[45px] absolute"
          viewBox="0 0 24 24"
    ...
    <div
      v-if="count"
      class="slider-counter relative mx-10 text-white flex items-center"
    >
      <span class="slider-current-index mx-3">{{
        useNumberFormatter(current)
      }}</span>
      <span class="slider-counter-delimiter h-[25px] border-r rotate-[30deg]" />
      <span class="slider-total-index mx-3 text-xl">{{
        useNumberFormatter(count)
      }}</span>
    </div>

    <!-- Next button -->
    <div
      class="swiper-circle cursor-pointer swiper-next group"
